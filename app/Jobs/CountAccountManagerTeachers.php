...
class CountAccountManagerTeachers extends Job implements SelfHandling, ShouldQueue
{
    ...
    {
        $umbrellaOrganizations = UmbrellaOrganization::where('user_id', $this->user->getKey())->select(['id', 'count_teachers'])->get();
        $count = 0;
        $umbrellaOrganizationIds = [];
        foreach ($umbrellaOrganizations as $umbrellaOrganization) {
            $count += $umbrellaOrganization->getAttribute('count_teachers');
        ...
        foreach ($schools as $school) {
            $count += $school->getAttribute('count_teachers');
