...
class CompletionQuestion extends Component
{
    ...
    private function completionHelper($question)
    {
        ...
        $replacementFunction = function ($matches) use ($question) {
            ...
            return sprintf(
                '<span><input x-on:contextmenu="$event.preventDefault()" spellcheck="false"    wire:model.lazy="answer.%d" class="form-input mb-2 truncate text-center overflow-ellipsis" type="text" id="%s" style="width: 120px" x-ref="%s" %s wire:key="%s"/>%s</span>',
                $tag_id,
