    ...
    private function multiHelper($question)
    {
        ...
        $question_text = preg_replace_callback(
            ...
            function ($matches) use ($tags, $isCitoQuestion,$question) {
                ...
                return sprintf('<select wire:model="answer.%s" class="form-input text-base max-w-full overflow-ellipsis overflow-hidden "  %s selid="testtake-select" x-ref="%s">%s</select>',
                    $matches[1],
                    $events,
