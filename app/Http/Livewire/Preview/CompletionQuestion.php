...
class CompletionQuestion extends Component
{
    ...
    private function completionHelper($question)
    {
        ...
        $replacementFunction = function ($matches) use ($question) {
            ...
            return sprintf(
                '<input wire:model.lazy="answer.%d" class="form-input mb-2 truncate text-center overflow-ellipsis" type="text" id="%s" style="width: 120px" x-ref="%s" @blur="$refs.%s.scrollLeft = 0"/>',
                $tag_id,
                'answer_' . $tag_id,
                'comp_answer_' . $tag_id,
                'comp_answer_' . $tag_id
