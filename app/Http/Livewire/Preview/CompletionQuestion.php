...
class CompletionQuestion extends Component
{
    ...
    private function completionHelper($question)
    {
        ...
        $replacementFunction = function ($matches) use ($question) {
            ...
            return sprintf(
                '<input wire:model.lazy="answer.%d" class="form-input mb-2 truncate text-center overflow-ellipsis" type="text" id="%s" style="width: 120px" x-ref="%s" @blur="$refs.%s.scrollLeft = 0" wire:key="%s"/>',
                $tag_id,
    ...
    private function multiHelper($question)
    {
        ...
        $question_text = preg_replace_callback(
            ...
            function ($matches) use ($tags, $isCitoQuestion) {
                ...
                return sprintf('<select wire:model="answer.%s" class="form-input text-base max-w-full overflow-ellipsis overflow-hidden" selid="testtake-select">%s</select>', $matches[1],
