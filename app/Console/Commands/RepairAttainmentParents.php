...
class RepairAttainmentParents extends Command
{
     ...
     * @var string
     */
    protected $signature = 'repair:attainment_parents';
    ...
    public function handle()
    {
        ...
        $collection->each(function($struct, $key) {
            ...
            if(!is_null($attainment)){
                $this->info(sprintf('parent already handled base_subject:%s, eductation_level:%s, code:%s',$struct["base_subject_name"],$struct["eductation_level_name"],$struct["code"]));
                return true;
            }
    ...
    private function makeParent($struct)
    {
        ...
        $fill = $struct->merge(['description'=>$description,'status'=>'ACTIVE']);
