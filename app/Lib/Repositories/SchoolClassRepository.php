...
class SchoolClassRepository {
    ...
    public static function getCompareSchoolClassToParallelSchoolClasses(SchoolClass $subjectSchoolClass) {
        ...
        $subjectIds = Teacher::where('class_id', $subjectSchoolClass->getKey())->distinct()->lists('subject_id')->all();
        ...
        foreach($allSchoolClasses as $schoolClassId => $schoolClass) {
                ...
                foreach ($wantedSchoolClassSubjects[$schoolClassId] as $subjectId) {
                    $schoolClassSubject = clone $subjects->whereLoose('id', $subjectId)->first();

                    $subjectTeachers = $teacherLines->whereLoose('class_id', $schoolClassId)->whereLoose('subject_id', $subjectId);
                    $subjectTeacherUsers = [];
                    foreach ($subjectTeachers as $teacher) {
                        $subjectTeacherUsers[] = $teacher->user;
                    }

                    $subjectAverages = $averages->whereLoose('school_class_id', $schoolClassId)->whereLoose('subject_id', $subjectId);
