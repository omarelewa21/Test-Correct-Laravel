...
class sys_net_Socket
{
    ...
    {
        sys_net_Socket::select(new _hx_array([$this]), null, null, null);
    }
    ...
    public function hpOfString($s)
    {
        $parts = _hx_explode(':', $s);
        if ($parts->length === 2) {
            return _hx_anonymous(['host' => new sys_net_Host($parts[0]), 'port' => Std::parseInt($parts[1])]);
        } else {
            return _hx_anonymous(['host' => new sys_net_Host(_hx_substr($parts[1], 2, null)), 'port' => Std::parseInt($parts[2])]);
        }
    }
