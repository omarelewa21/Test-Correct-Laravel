...
class com_wiris_system_CallWrapper
{
        ...
        if (! class_exists('php_Boot', false)) {
            ...
            if (is_file($haxelib.'/cache/haxe_autoload.php')) {
                ...
                if (! function_exists('_hx_wiris_load')) {
                    function _hx_wiris_load($d, $pack = [])
                    {
                        $h = opendir($d);
                        while (false !== ($f = readdir($h))) {
                            ...
                            if (is_file($p) && substr($f, -4) == '.php') {
                                ...
                                if (substr($bn, -6) == '.class') {
                                    $bn = substr($bn, 0, -6);
                                    $t = '_hx_class';
                                } elseif (substr($bn, -5) == '.enum') {
                                    $bn = substr($bn, 0, -5);
                                    $t = '_hx_enum';
                                } elseif (substr($bn, -10) == '.interface') {
                                    $bn = substr($bn, 0, -10);
                                    $t = '_hx_interface';
                                } elseif (substr($bn, -7) == '.extern') {
                                    $bn = substr($bn, 0, -7);
                                    $t = '_hx_class';
                                    $name = $bn;
                                } else {
                                ...
                                $qname = ($bn == 'HList' && empty($pack)) ? 'List' : implode('.', array_merge($pack, [$bn]));
                                $phpname = ! empty($name) ? $name : implode('_', array_merge($pack, [$bn]));
                                _hx_register_type(new $t($phpname, $qname, $p));
                            } elseif (is_dir($p)) {
                                _hx_wiris_load($p, array_merge($pack, [$f]));
                            }
                        }
                    }
                }
    ...
    {
        return 'com.wiris.system.CallWrapper';
    }
}
