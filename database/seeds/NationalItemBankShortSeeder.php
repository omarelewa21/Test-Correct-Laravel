...
class NationalItemBankShortSeeder extends Seeder
{
    /**
    ...
    {
        $oldUser = Auth::check() ? Auth::user() : false;
        ...
        BaseSubject::limit(2)->orderBy('id')->get()->each(function ($baseSubject) use ($teacherA, $teacherB, $class, $classB, $section, $periodLocationA) {
            ...
            $test = FactoryScenarioTestTestWithOpenShortQuestion::create("TBNI $subject->name", $teacherA)->getTestModel();
            $test->fill([
                'subject_id'             => $subject->id,
                'education_level_id'     => 1,
        ...
        $oldUser ? Auth::login($oldUser) : Auth::logout();
    }
}
