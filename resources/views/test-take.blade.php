<x-layouts.app>
    <div class="w-full flex flex-col mb-5" selid="testtake-layout">
        @if($testParticipant->intense)
            <livewire:student.intense-observer :deviceId="$testParticipant->user_id"
                                               :sessionId="$testParticipant->id"></livewire:student.intense-observer>
        @endif
        ...
        <div class="test-take-questions">
            ...
            @foreach($data as  $key => $testQuestion)
                <div selid="testtake-question" class="test-take-question-{{ $key + 1 }}">
                    <livewire:is :component="$testQuestion->getStudentPlayerComponent()"
        ...
        <x-slot name="footerbuttons">
            ...
            >
                <x-button.text x-show="display.prev"
                               x-on:click="$store.studentPlayer.previous(current)"
                               href="#"
                               rotateIcon="180">
                    <x-icon.chevron />
                ...
                <x-button.primary x-show="display.next"
                                  x-on:click="$store.studentPlayer.next(current)"
                                  size="sm">
