<x-partials.question-container :number="$number" :question="$question">
    <div class="flex flex-col" x-on:start-timeout.window="alert($event.detail)">
    locked: {{ $locked }}
        <div x-data="{opened: @entangle('drawingModalOpened'), answered: @entangle('answer')}"
             class="relative">
            <div class="flex flex-col space-y-3">
                <span>Maak een tekening vraag. @if(!$question->attachments->isEmpty())Bekijk ook de bijlagen bij deze
                    vraag. @endif @if($question->note_type != "NONE") Open het notitieblok om aantekeningen te
                    noteren. @endif</span>
