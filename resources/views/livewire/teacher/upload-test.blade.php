<div class="flex flex-col w-full upload-test min-h-screen">
    ...
    @if(tcCore\Http\Helpers\SchoolHelper::isTempTeachersSchoolLocation(Auth::user()->schoolLocation))
                ...
                <x-accordion.block :key="1" :emitWhenSet="true">
                    <x-slot name="title">
                        <div class="flex gap-2 items-center">
                            <h4 class="group-hover:text-primary  transition-colorstransition-colors">@lang('upload.Remark')</h4>
                        </div>
                    </x-slot>

                    <x-slot name="body">
                        <div class="grid grid-flow-row w-full gap-4">
                            @lang('upload.Not allowed due to temp school')
                        </div>
                    </x-slot>
                </x-accordion.block>
