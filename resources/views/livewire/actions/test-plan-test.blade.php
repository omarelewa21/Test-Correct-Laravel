<div class="{{ $class ?? '' }}">
    ...
    @elseif($variant == 'icon-button')
        @if($disabled)
            <x-button.icon wire:key='test-plan-test-{{ $uuid }}' color="cta opacity-20 cursor-not-allowed" title="{{ __('teacher.Inplannen') }}">
                <x-icon.schedule/>
            </x-button.icon>
        @else
            <x-button.icon wire:key='test-plan-test-{{ $uuid }}' color="cta" wire:click="handle" title="{{ __('teacher.Inplannen') }}">
                <x-icon.schedule/>
            </x-button.icon>
        @endif
