...
<div class="question-button flex items-center cursor-pointer bold py-2 @if($subQuestion === 'false') px-6 @endif @if($question->uuid === $this->testQuestionId) primary active @endif"
            ...
            >
                ...
                >
                    ...
                    <div x-cloak
                         ...
                         class="absolute flex flex-col -right-5 top-10 bg-white text-sysbase py-2 main-shadow rounded-10 w-72 z-10"
...
{{--                        </button>--}}
                        <div class="px-4 py-2 flex text-sysbase" title="{{ __('cms.Verwijderen') }}">
                            <x-icon.trash/> {{ __('cms.Verwijderen') }}
                        </div>
                        <div
                                class="px-4 py-2 flex text-sysbase"
                                title="{{ __('cms.Wijzigen') }}"
                                wire:click="showQuestion('{{ $testQuestion ? $testQuestion->uuid : null }}', '{{ $question->uuid }}', false)"
                                @click="$dispatch('question-change', {old: '{{ $this->testQuestionId }}', new: '{{ $question->uuid }}' })"
                        >
                            <x-icon.edit/> {{ __('cms.Wijzigen') }}
                        </div>
                    </div>
                </button>
            </div>
