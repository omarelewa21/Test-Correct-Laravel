...
<span id="{{ $containerId }}"
      ...
      wire:ignore
      x-data="singleSelect(@js($containerId),@if($attributes->wire('model')->value) @entangle($attributes->wire('model')->value){{ $defer }} @else null @endif , @js($disabled), @js($error))"
      x-cloak
      x-on:click="toggleDropdown()"
      x-on:select-option=""
      x-on:disable-single-select="disableDropdown()"
      x-on:enable-single-select="enableDropdown()"
      x-on:enable-error-state="errorState = true"
      x-on:disable-error-state="errorState = false"
      x-bind:class="{'border-primary text-primary': singleSelectOpen, 'disabled pointer-events-none bg-white border-lightGrey text-midgrey': singleSelectDisabled, 'border-allred error': errorState, 'border-bluegrey': !errorState }"
      x-bind:aria-expanded="singleSelectOpen ? 'true' : 'false'"
      x-bind:data-selected-value="value ?? 'none'"
      {{ $attributes->except(['id','class', 'wire:model']) }}
>
    <span class="selected truncate font-normal"
          x-bind:class="{'italic': ['', null].includes(value), 'text-sysbase': !['', null].includes(value) && !singleSelectDisabled, 'text-midgrey': ['', null].includes(value) || singleSelectDisabled }"
