<span class="flex items-center flex-wrap co-learning-completion">
    @foreach($questionTextPartials as $answerIndex => $textPartialArray)
        @foreach($textPartialArray as $textPartial)
            {!!$textPartial!!}
        @endforeach
        <span class="inline-flex mx-2 relative top-1 mb-2 px-2.5 py-0.5 rounded-10 bg-offwhite border border-bluegrey bold items-center gap-1 w-min align-middle">
