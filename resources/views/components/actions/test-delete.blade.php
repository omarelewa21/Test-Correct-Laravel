<div>
    @if($variant == 'icon_button')
        @if ($test->canDelete(auth()->user()))
            <x-button.primary
                    class="pl-[12px] pr-[12px]"
                    @click="$emitTo('teacher.test-delete-modal', 'displayModal', '{{  $test->uuid }}')">
                <x-icon.trash/>
            </x-button.primary>
        @else
            <x-button.primary
    ...
    @elseif($variant == 'context-menu')
        @if( $test->canDelete(auth()->user()))
            <button
                    ...
                    @click="$event.target.dispatchEvent(new CustomEvent('context-menu-close', { bubbles: true }));$wire.emitTo('teacher.test-delete-modal', 'displayModal', '{{ json_encode(['testUuid' => $test->uuid]) }} ')"
            >
                <x-icon.remove/>
