<span x-data="initReadSpeakerLanguage()" style="position: relative; top:-16px; isolation: isolate">
      @foreach(['de', 'en', 'fr', 'nl', 'es'] as $language)
        <button
            ...
            class="rs_active_flag_button relative bg-white border-1 border-primary  px-2 py-1 rounded-full h-[40px] w-[40px] z-10"
        ...
        >
            <div
                ...
                class="rs_flag_absolute w-8 h-8 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2"
    ...
    @endforeach
        <div
            ...
            class="rs_language_menu absolute z-[5] w-[40px] bg-white rounded-t-full rounded-b-full shadow-lg"
        ...
        @foreach(['de', 'en', 'fr', 'nl', 'es'] as $language)
                <button
                    ...
                    class="rs_flag px-2 py-1 rounded-full h-[40px] w-[40px] bg-white relative"
                    title="{{__('test_take.language.'.$language)}}"

                >
                     <div
                         x-show="isHovering"
                         class="rs_flag_highlight w-8 h-8 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2"
                         style="border-color: rgba(0, 77, 245, 0.5); border-width: 1px; border-style: solid; border-radius: 50%; ;"
